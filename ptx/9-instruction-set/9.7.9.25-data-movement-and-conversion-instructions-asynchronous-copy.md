...
The copy operation in `cp.async.bulk` is treated as a weak memory operation and the complete-tx operation on the mbarrier has `.release` semantics at the `.cluster` scope as described in the Memory Consistency Model. The copy operation is performed in the async proxy.
