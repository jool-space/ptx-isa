...
The immediate integer operand `ord` specifies the ordinal of the field across the rank of the tensor which needs to be replaced in the tensor-map object. The only valid values for `ord` are `[0..4]`.
...
When `.field3` is specified, the operand `new_val` must be an immediate and the Table 36 shows the mapping of the operand `new_val` across various fields.


Table 36 Tensormap new_val validity **new_val** | **.field3**  
---|---  
