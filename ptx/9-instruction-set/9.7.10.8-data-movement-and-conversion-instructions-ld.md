...
The optional qualifier `.proxy::readonly` may be used to indicate that the memory operation is performed using the readonly proxy. When this qualifier is specified, the memory location referred to by source operand `a` is treated as read-only during the lifetime of the kernel. This qualifier is allowed when source address `a` is in `.global` state space or is a generic address that points to `.global` or `.const` state space, otherwise behavior is undefined.


The semantic details of `.mmio` qualifier are described in the Memory Consistency Model. Only `.sys` thread scope is valid for `ld.mmio` operation.
