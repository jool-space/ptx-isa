...
The semantic details of `.mmio` qualifier are described in the Memory Consistency Model. Only `.sys` thread scope is valid for `st.mmio` operation.
...
A source register wider than the specified type may be used. The lower `n` bits corresponding to the instruction-type width are stored to memory. See Table 27 for a description of these relaxed type-checking rules.
